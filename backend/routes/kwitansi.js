...
// Setup upload directory untuk file SPD
const spdUploadDir = path.join(__dirname, '../public/uploads/spd');
if (!fs.existsSync(spdUploadDir)) {
    fs.mkdirSync(spdUploadDir, { recursive: true });
    console.log('✅ SPD upload directory created:', spdUploadDir);
}

// Konfigurasi multer untuk upload file SPD
const spdStorage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, spdUploadDir);
    },
    filename: (req, file, cb) => {
        const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
        const ext = path.extname(file.originalname);
        const filename = 'spd-' + uniqueSuffix + ext;
        cb(null, filename);
    }
});
...
router.get('/:id', keycloakAuth, async (req, res) => {
    try {
        ...
        if (!roleInfo.isAdmin && !roleInfo.isPPK && !roleInfo.isBendahara) {
            // Creator kegiatan (pembuat nominatif) juga boleh melihat kwitansi pegawai lain.
            // Untuk pegawai, gunakan pencocokan fleksibel agar NIP parsial di DB cocok dengan NIP lengkap user.
            query += ` AND (
                n.user_id = ?
                OR REPLACE(p.nip, ' ', '') = ?
            ...
            params.push(getUserId(user), userNip, userNip, userNip);
        }
    ...
    try {
        ...
        const [existingKwitansi] = await db.query(`
            SELECT k.*, p.nip as pegawai_nip, p.nama as pegawai_nama,
                   n.ppk_nip, n.bendahara_nip, n.user_id as kegiatan_creator_id
            FROM kwitansi_perjadin k
            JOIN nominatif_pegawai p ON k.pegawai_id = p.id
            JOIN nominatif_kegiatan n ON k.kegiatan_id = n.id
            WHERE k.id = ?
        ...
        // Pembuat nominatif (creator kegiatan) juga boleh memperbaiki kwitansi pegawai lain,
        // terutama setelah kwitansi ditolak PPK / Bendahara.
        const isCreator = kwitansi.kegiatan_creator_id === getUserId(user);
        
        // Gunakan matchNip agar NIP parsial di DB (mis. 804960002) cocok dengan NIP lengkap user (6271030804960002)
        const canUpdate = roleInfo.isAdmin || isCreator || matchNip(normalizedUserNip, normalizedPegawaiNip);
        ...
        // Izinkan update jika: admin/creator, atau pegawai sendiri dan (belum approve atau ditolak)
        if (!roleInfo.isAdmin && !isCreator && !isPegawaiBelumApprove && !isRejected) {
            return res.status(400).json({ 
                success: false, 
...
// Helper hapus file SPD lama dari disk
function removeSpdFile(storedPath) {
    if (!storedPath) return;
    const filename = String(storedPath).split('/').pop();
    if (!filename) return;
    const fullPath = path.join(spdUploadDir, filename);
    fs.unlink(fullPath, (err) => {
        if (err && err.code !== 'ENOENT') {
            console.error('⚠️ Gagal hapus file SPD lama:', err.message);
        }
    });
}

// Cek akses untuk upload/hapus file SPD pada kwitansi
async function checkSpdAccess(kwitansiId, user) {
    const userNip = user?.nip || '';
    const roleInfo = getUserRoleInfo(user);
    const normalizedUserNip = normalizeNip(userNip);

    const [rows] = await db.query(`
        SELECT k.*, p.nip as pegawai_nip, n.user_id as kegiatan_creator_id
        FROM kwitansi_perjadin k
        JOIN nominatif_pegawai p ON k.pegawai_id = p.id
        JOIN nominatif_kegiatan n ON k.kegiatan_id = n.id
        WHERE k.id = ?
    ...
    if (rows.length === 0) {
        return { error: { status: 404, message: 'Kwitansi tidak ditemukan' } };
    }

    const kwitansi = rows[0];
    const normalizedPegawaiNip = normalizeNip(kwitansi.pegawai_nip);
    // Admin, pegawai yang bersangkutan, atau pembuat nominatif (creator) boleh mengelola file SPD
    const isCreator = kwitansi.kegiatan_creator_id === getUserId(user);
    const canUpdate = roleInfo.isAdmin || isCreator || matchNip(normalizedUserNip, normalizedPegawaiNip);

    if (!canUpdate) {
        return { error: { status: 403, message: 'Tidak memiliki akses untuk mengunggah file SPD kwitansi ini' } };
    }
    ...
    if (!roleInfo.isAdmin && !isCreator && !isPegawaiBelumApprove && !isRejected) {
        return { error: { status: 400, message: 'File SPD tidak dapat diubah karena kwitansi sudah dalam proses approval' } };
    }

    return { kwitansi };
}

// ============ POST upload file SPD ============
router.post('/spd/:kwitansiId', keycloakAuth, uploadSpd.single('upload_spd'), async (req, res) => {
    try {
        const { kwitansiId } = req.params;
        const user = req.user;

        const { error, kwitansi } = await checkSpdAccess(kwitansiId, user);
        if (error) {
            return res.status(error.status).json({ success: false, message: error.message });
        }

        if (!req.file) {
            return res.status(400).json({ success: false, message: 'File SPD wajib diunggah' });
        }

        // Hapus file SPD lama jika ada
        if (kwitansi.upload_spd) {
            removeSpdFile(kwitansi.upload_spd);
        }

        const filePath = `/uploads/spd/${req.file.filename}`;

        await db.query(`
            UPDATE kwitansi_perjadin
            SET upload_spd = ?, updated_at = NOW()
            WHERE id = ?
        `, [filePath, kwitansiId]);

        console.log(`✅ File SPD uploaded untuk kwitansi ID ${kwitansiId}: ${filePath}`);

        res.status(200).json({
            success: true,
            data: { id: kwitansiId, upload_spd: filePath },
            message: 'File SPD berhasil diunggah'
        });
    } catch (error) {
        console.error('❌ Error uploading SPD file:', error);
        res.status(500).json({ success: false, message: error.message });
    }
});

// ============ DELETE hapus file SPD ============
router.delete('/spd/:kwitansiId', keycloakAuth, async (req, res) => {
    try {
        const { kwitansiId } = req.params;
        const user = req.user;
        ...
        await db.query(`
            UPDATE kwitansi_perjadin
            SET upload_spd = NULL, updated_at = NOW()
            WHERE id = ?
        `, [kwitansiId]);

        console.log(`🗑️ File SPD dihapus untuk kwitansi ID ${kwitansiId}`);

        res.status(200).json({
            success: true,
            message: 'File SPD berhasil dihapus'
        });
    } catch (error) {
        console.error('❌ Error deleting SPD file:', error);
        res.status(500).json({ success: false, message: error.message });
    }
});
