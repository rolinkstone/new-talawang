...
const generateSptjmPenginapanRows = (dataSource = null) => {
  ...
  const handlePrintKwitansi = async () => {
    ...
    // File SPD (upload saat input kwitansi) — ikut digabung ke dokumen
    const spdFilePath = item?.upload_spd || pegawai?.upload_spd || null;
    let spdUrl = null;
    let spdIsImage = false;
    if (spdFilePath) {
      spdUrl = BACKEND_URL + spdFilePath;
      const spdExt = (String(spdFilePath).split('?')[0].split('.').pop() || '').toLowerCase();
      spdIsImage = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp'].includes(spdExt);
      if (!spdIsImage) {
        // PDF/Word dirender lewat pdf.js seperti lampiran lainnya
        allFileUrls.push({ id: 'spd', url: spdUrl, name: String(spdFilePath).split('/').pop(), type: 'spd' });
      }
    }
    ...
    // Build HTML untuk lampiran file SPD
    let spdFilesHtml = '';
    if (spdFilePath) {
      if (spdIsImage) {
        spdFilesHtml = '<div style="margin: 10px 0; text-align: center;"><img src="' + spdUrl + '" style="max-width: 100%; border: 1px solid #ccc;" onerror="this.parentElement.innerHTML=\'<em>(File SPD tidak dapat ditampilkan)</em>\';" /></div>';
      } else if (renderedPages['spd'] && renderedPages['spd'].length > 0) {
        renderedPages['spd'].forEach(pageData => {
          spdFilesHtml += '<div style="margin: 10px 0; text-align: center;"><img src="' + pageData + '" style="max-width: 100%; border: 1px solid #ccc;" /></div>';
        });
      } else {
        spdFilesHtml = '<div style="margin: 10px 0; text-align: center;"><em>(File SPD tidak dapat ditampilkan di pratinjau. Buka file <a href="' + spdUrl + '" target="_blank">di sini</a>.)</em></div>';
      }
    }
    ...
    printWindow.document.write(`
      ...
      <body>
        ...
        <div class="page-break"></div>
        <div class="sptjm-container">
          <div style="text-align: center; font-size: 13px; font-weight: bold; margin-bottom: 15px;">LAMPIRAN FILE SPD</div>
          ${spdFilesHtml}
        </div>
        ` : ''}
