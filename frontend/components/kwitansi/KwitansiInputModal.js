...
export default function KwitansiInputModal({ kegiatan, pegawai, onClose, onSuccess }) {
    ...
    // File SPD (opsional) - untuk melampirkan Surat Perintah Dinas
    const [spdUpload, setSpdUpload] = useState({
        original: pegawai?.upload_spd || null, // file SPD yang tersimpan di DB (mode edit)
        current: pegawai?.upload_spd || null,  // file yang sedang aktif di form
        newFile: null                          // file baru yang dipilih user
    });
    ...
    // ============ Upload File SPD ============
    const buildSpdFileUrl = (storedPath) => {
        if (!storedPath) return null;
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api';
        return `${apiUrl}${String(storedPath).replace(/^\/api/, '')}`;
    };
    
    const handleSpdFileChange = (e) => {
        ...
        setSpdUpload(prev => ({ ...prev, newFile: file }));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                // 1b. Upload / hapus File SPD (opsional)
                if (spdUpload.newFile) {
                    const spdFormData = new FormData();
                    spdFormData.append('upload_spd', spdUpload.newFile);
                    
                    await axios.post(
                        `${process.env.NEXT_PUBLIC_API_URL}/kwitansi/spd/${kwitansiId}`,
                        spdFormData,
                        {
                            headers: {
                                Authorization: `Bearer ${session.accessToken}`,
                ...
                } else if (spdUpload.original && !spdUpload.current) {
                    // User menghapus file SPD yang sudah ada sebelumnya
                    await axios.delete(
                        `${process.env.NEXT_PUBLIC_API_URL}/kwitansi/spd/${kwitansiId}`,
                        {
                            headers: {
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            <div>
                                ...
                                {/* Upload File SPD (opsional) */}
                                <div className="mt-3">
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Upload File SPD</label>
                                    <input
                                        type="file"
                                        accept="image/*,.pdf,.doc,.docx"
                                        onChange={handleSpdFileChange}
                                        className="block w-full text-sm text-gray-600 dark:text-gray-300 file:mr-3 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-indigo-600 file:text-white hover:file:bg-indigo-700 file:cursor-pointer"
                                    />
                                    {(spdUpload.current || spdUpload.newFile) && (
                                        <div className="mt-2 flex items-center justify-between gap-2 bg-gray-100 dark:bg-gray-700 rounded-md px-3 py-2">
                                            <div className="flex items-center gap-2 min-w-0">
                                                ...
                                                <span className="truncate text-sm text-gray-700 dark:text-gray-200">
                                                    {spdUpload.newFile ? spdUpload.newFile.name : (spdUpload.current || '').split('/').pop()}
                                                </span>
                                                {!spdUpload.newFile && spdUpload.current && (
                                                    <a
                                                        href={buildSpdFileUrl(spdUpload.current)}
                                                        target="_blank"
                                    ...
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Opsional. Format: JPG, PNG, PDF, Word (Maks 10MB)</p>
                                </div>
                            </div>
