...
export default function KwitansiContainer() {
    ...
    const filterOtherData = (data) => {
        ...
        const filtered = data.filter(kegiatan => {
            const isCreatorKegiatan = kegiatan.isCreator === true ||
                                      (!!currentUserId && String(kegiatan.user_id) === String(currentUserId));
    ...
    const fetchNeedKwitansi = async () => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                const processedData = res.data.data.map(kegiatan => {
                    ...
                    const pegawaiWithFlag = pegawaiList.map(pegawai => {
                        const isCurrentUser = matchNip(pegawai.nip, currentUserNip) ||
                                              ...
                                              (!!kegiatan.pegawai_id && pegawai.id === kegiatan.pegawai_id);
                        return {
                            ...pegawai,
                    ...
                    // Pakai flag dari backend (is_creator) agar tetap akurat walau format user_id berbeda
                    const isCreatorKegiatan = kegiatan.is_creator === true ||
                                              (!!currentUserId && String(kegiatan.user_id) === String(currentUserId));
                ...
                // Jika user bukan peserta di kegiatan manapun tetapi dia pembuat nominatif (creator),
                // langsung arahkan ke tab "Pegawai Lain" agar kwitansi pegawai lain mudah ditemukan.
                const hasSelfData = processedData.some(k =>
                    (k.pegawai || []).some(p => p.isCurrentUser === true)
                );
                const nextTab = (hasCreatorAccess && !hasSelfData && activeTab === 'diri_sendiri')
                    ? 'pegawai_lain'
                    : activeTab;
    ...
    const handleViewDetail = async (pegawai, kegiatan) => {
        try {
            ...
            setSelectedKwitansi({
                ...
                is_creator_kegiatan: kegiatan.isCreator === true || (!!currentUserId && String(kegiatan.user_id) === String(currentUserId)),
                kegiatan_user_id: kegiatan.user_id,
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        ...
                                        <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                            {kegiatan.pegawai.map((pegawai, idx) => {
                                                ...
                                                // Bisa edit bila belum terkunci approval, dan user adalah pegawai ybs / pembuat nominatif (creator) / admin
                                                const isLockedAfterApproval = pegawai.status_pegawai === 'sudah' &&
                                                                               pegawai.status_ppk !== 'ditolak' &&
                                                                               pegawai.status_bendahara !== 'ditolak';
                                                const canEditKwitansi = !isLockedAfterApproval &&
                                                                        (pegawai.isCurrentUser === true || kegiatan.isCreator === true || userType.isAdmin === true);
                                                        ...
                                                        <td className="px-4 py-3 text-center">
                                                            <div className="flex justify-center gap-2 flex-wrap">
                                                                {!sudahInput ? (
                                                                    ...
                                                                    <>
                                                                        {/* Tombol Edit — tampil jika belum terkunci approval dan user = pegawai ybs / pembuat nominatif (creator) / admin */}
                                                                        {canEditKwitansi && (
                                                                            <button 
                                                                                onClick={() => handleInputKwitansi(kegiatan, pegawai)} 
