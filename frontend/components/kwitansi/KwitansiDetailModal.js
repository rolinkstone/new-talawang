...
export default function KwitansiDetailModal({ item, onClose, formatDateForDisplay, formatRupiah, onRefresh }) {
    ...
    // Cek apakah kwitansi ditolak dan user berhak memperbaikinya
    ...
    const isCreator = item.is_creator_kegiatan === true ||
                      (!!item.kegiatan_user_id && !!session?.user?.id && String(item.kegiatan_user_id) === String(session.user.id));
    const canEdit = (isPegawai || isCreator) && isRejected;
    ...
    const handleEditSubmit = async () => {
        ...
        try {
            ...
            // Kirim JSON: endpoint PUT /kwitansi/:id hanya memproses no_lpd, tgl_kwitansi, tgl_spd
            const response = await axios.put(
                `${process.env.NEXT_PUBLIC_API_URL}/kwitansi/${kwitansiId}`,
                {
                    no_lpd: editData.no_lpd,
                    tgl_kwitansi: editData.tgl_kwitansi,
                    tgl_spd: editData.tgl_spd
                },
                {
                    headers: {
    ...
    // ===== File SPD (yang di-upload pada saat input kwitansi) =====
    const spdApiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api';
    const spdStoredPath = item?.upload_spd || null;
    const spdFileUrl = spdStoredPath ? `${spdApiUrl}${String(spdStoredPath).replace(/^\/api/, '')}` : null;
    const spdFileName = spdStoredPath ? String(spdStoredPath).split('/').pop() : '';

    const renderSpdView = () => {
        if (!spdFileUrl) return null;
        return (
            <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg flex items-center justify-between gap-3">
                <div className="flex items-center gap-2 flex-1 min-w-0">
                    {getFileIcon(spdFileName)}
                    <div className="min-w-0">
                        <p className="text-sm font-medium text-gray-700 dark:text-gray-200">File SPD</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{spdFileName}</p>
                    </div>
                </div>
                <a
                    href={spdFileUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-3 py-1.5 bg-blue-600 text-white text-xs rounded-md hover:bg-blue-700 whitespace-nowrap shrink-0"
                >
                    Lihat
                </a>
            </div>
