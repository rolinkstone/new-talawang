      ...
      {(filters.bulan !== 'all' || filters.pegawai_id !== 'all' || filters.status_2 !== 'all') && (
        <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
          <div className="flex flex-wrap gap-2">
            {filters.bulan !== 'all' && (
              <span className="inline-flex items-center gap-1 px-2 py-1 bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-200 rounded-full text-xs">
                Bulan: {bulanOptions.find(b => b.value === filters.bulan)?.label}
                <button onClick={() => handleFilterChange('bulan', 'all')} className="hover:text-blue-900 dark:hover:text-blue-100">
                  <FaTimes className="text-xs" />
                </button>
              </span>
            )}
            {filters.pegawai_id !== 'all' && (
              <span className="inline-flex items-center gap-1 px-2 py-1 bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-200 rounded-full text-xs">
                Pegawai: {options.pegawai?.find(p => p.id === parseInt(filters.pegawai_id))?.nama}
                <button onClick={() => handleFilterChange('pegawai_id', 'all')} className="hover:text-green-900 dark:hover:text-green-100">
                  <FaTimes className="text-xs" />
                </button>
              </span>
            )}
            {filters.status_2 !== 'all' && filters.status_2 !== 'selesai' && (
              <span className="inline-flex items-center gap-1 px-2 py-1 bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-200 rounded-full text-xs">
                Status: {filters.status_2}
                <button onClick={() => handleFilterChange('status_2', 'selesai')} className="hover:text-purple-900 dark:hover:text-purple-100">
                  <FaTimes className="text-xs" />
                </button>
              </span>
            )}
